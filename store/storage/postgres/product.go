...
func (p *productRepo) Search(customerProductIDs map[string]int) (map[string]int, map[string]int, error) {
	var (
		...
		//productQuantities = make([]int, len(customerProductIDs))
		products          []string
		productQuantities []int
		productPrices     = make(map[string]int, 0)
	)
	//customer kiritgan productlani idisini slicega append qivotdi
	for key, value := range customerProductIDs {
		products = append(products, key)
		productQuantities = append(productQuantities, value)
	}

	query := `select id, quantity, price, original_price 
			from products where id::varchar = ANY($1)
				and quantity >= ALL($2);
`

	rows, err := p.db.Query(context.Background(), query, pq.Array(products), pq.Array(productQuantities))
	if err != nil {
		fmt.Println("error is while getting products by product ids", err.Error())
		return nil, nil, err
	}
	...
	return selectedProducts.Products, productPrices, err
}
