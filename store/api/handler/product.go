...
// @Accept       json
// @Produce      json
// @Param 		 sell body models.SellRequest false "sell"
// @Success      200  {object}  models.Response
// @Failure      400  {object}  models.Response
// @Failure      404  {object}  models.Response
// @Failure      500  {object}  models.Response
func (h Handler) StartSellNew(c *gin.Context) {
	...
	if err != nil {
		handleResponse(c, "error is while searching products", http.StatusInternalServerError, err.Error())
		return
	}

	basket, err := h.storage.Basket().GetByID(models.PrimaryKey{ID: request.BasketID})
	if err != nil {
		handleResponse(c, "error is while getting basket", http.StatusInternalServerError, err.Error())
		return
	}

	customer, err := h.storage.User().GetByID(models.PrimaryKey{ID: basket.CustomerID})
	if err != nil {
		handleResponse(c, "error is while getting customer", http.StatusInternalServerError, err.Error())
		return
	}
	...
	if err = h.storage.Product().TakeProducts(basketProducts); err != nil {
		handleResponse(c, "error is while taking products", http.StatusInternalServerError, err.Error())
		return
	}

	if err = h.storage.BasketProduct().AddProducts(basket.ID, basketProducts); err != nil {
		handleResponse(c, "error is while adding products", http.StatusInternalServerError, err.Error())
		return
	}
	//save profit in db

	handleResponse(c, "succesfully finished the products", http.StatusOK, profit)
}
